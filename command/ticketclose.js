...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const filter = (btnInteraction) => {
      return ['confirm_close', 'cancel_close'].includes(btnInteraction.customId) && btnInteraction.user.id === interaction.user.id;
    };
    ...
    collector.on('end', async (collected) => {
      if (collected.size === 0) {
        await interaction.editReply({ content: 'Ticket close timed out.', components: [] });
      } else {
